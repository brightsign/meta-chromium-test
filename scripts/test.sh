...
if [ $# -le 3 ]; then
  ...
  echo E.g. $0 scarthgap x86-64 ozone-wayland
  exit 1
fi
...
if [ -z "$libc_flavour" ]; then
  image_folder=glibc-$yocto_version-$chromium_version-$arch
else
  image_folder=$libc_flavour-$yocto_version-$chromium_version-$arch
fi
